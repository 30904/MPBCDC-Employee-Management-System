...
export default function LoanApprovalWorkflow() {
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      const { items } = await fetchApprovalMatrices({ module: 'LOAN', limit: 100 });
  ...
  async function handleInitialize() {
    ...
    try {
      await initializeLoanApprovalMatrix();
      await loadRows();
    } catch (err) {
  ...
  async function handleDelete(row) {
    if (!window.confirm(`Delete level ${row.level} (${row.approverRole})?`)) {
      return;
    }
